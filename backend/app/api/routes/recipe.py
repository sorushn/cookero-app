...
@recipe_router.post(
    "/recipe",
    tags=["Recipes"],
    responses={200: {"model": RecipeCreateResponse}, 400: {"model": str}},
)
async def create_recipe(
    request: RecipeCreate, current_user_id: str = Depends(get_current_user_id)
) -> dict:
...
async def get_recipe(
    id: str, current_user_id: str = Depends(get_current_user_id)
) -> dict:
...
async def update_recipe(
    id: str, request: RecipeUpdate, current_user_id: str = Depends(get_current_user_id)
) -> dict:
    ...
    if "error" in recipe:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=recipe["error"]
        )
...
@recipe_router.delete(
    "/recipe/{id}",
    tags=["Recipes"],
    responses={200: {"model": str}, 400: {"model": str}},
)
...
@recipe_router.get(
    "/recipes",
    tags=["Recipes"],
    responses={200: {"model": list[RecipeCreateResponse]}, 400: {"model": str}},
)
