...
def test_create_recipe(temp_recipe, db_user):
...
def test_update_recipe(db_recipe, temp_recipe):
    update_data = {
...
def test_delete_recipe(db_recipe):
...
def test_get_recipes_by_user_id(db_user, db_recipe):
    response = client.get(f"/recipes/{db_user['id']}")
    assert response.status_code == 200
    data = response.json()
    logger.debug(data)
    assert len(data) == 1
